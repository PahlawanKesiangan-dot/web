...
Website pemesanan percetakan berbahasa Indonesia dengan formulir pelanggan, upload banyak lampiran dalam satu pesanan, penyimpanan data pesanan dan seluruh lampiran di Firebase Realtime Database, serta panel admin yang dilindungi Firebase Authentication. Lampiran PNG/JPG/JPEG/PDF/DOCX/XLSX disimpan sebagai Base64; Firebase Storage tidak digunakan.
...
File pelanggan tidak melewati Vercel Functions saat diunggah. Browser memperoleh izin Firebase sementara dan menulis setiap lampiran Base64 ke lokasi Realtime Database yang sudah ditentukan. Server lalu memeriksa metadata, ukuran, tipe MIME, Base64, dan signature awal setiap file sebelum membuat pesanan. Isi file dipisahkan pada jalur `orderFiles/` agar daftar `orders/` tetap ringan.
...
3. Framework akan terdeteksi sebagai Next.js. Script produksi memakai `next build --webpack` dan `next.config.ts` memasukkan `firebase-admin` ke bundle server agar modulnya tersedia di Vercel Functions.
...
Jika runtime log menampilkan `Failed to load external module firebase-admin-<hash>`, pastikan repository memuat kedua perubahan berikut, lalu deploy commit terbaru:

- `package.json`: script `build` bernilai `next build --webpack`;
- `next.config.ts`: `transpilePackages: ['firebase-admin']` berada di dalam objek konfigurasi.

Log lama tetap tersimpan di Vercel. Saat memeriksa hasil perbaikan, filter log berdasarkan deployment atau hostname terbaru.
...
- melihat jumlah lampiran dan mengunduh setiap file yang direkonstruksi dari data Base64;
...
Kontak awal website adalah WhatsApp `0851 2400 5652` (`6285124005652` untuk tautan WhatsApp). Nomor ini dapat diubah melalui **Admin > Pengaturan > Informasi bisnis**.
...
- Jumlah lampiran dalam satu pesanan tidak diberi batas tetap. Setiap file dibatasi maksimal 7 MB karena batas string Base64 Realtime Database; Base64 berukuran sekitar sepertiga lebih besar daripada file biner.
