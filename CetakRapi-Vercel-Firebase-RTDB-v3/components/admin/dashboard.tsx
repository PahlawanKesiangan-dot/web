...
import { orderStatuses, statusLabels, type OrderRecord, type OrderStatus } from '@/lib/order-types';
...
export function AdminDashboard({ orders: initial, settings, adminName }: { orders: OrderRecord[]; settings: PublicSettings; adminName: string }) {
  ...
  return <main className="min-h-screen bg-slate-100 text-slate-950">
    <header className="border-b bg-[var(--ink)] text-white"><div className="mx-auto flex max-w-[1440px] items-center justify-between gap-4 px-5 py-4 sm:px-8"><a href="/admin" className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-[var(--cyan)] text-[var(--ink)]"><ShoppingBag className="size-5" /></span><span><strong className="block">CetakRapi Admin</strong><small className="block text-slate-300">{adminName}</small></span></a><div className="flex items-center gap-2"><Button render={<a href="/" />} variant="ghost" className="text-white hover:bg-white/10 hover:text-white">Website pelanggan</Button><Button render={<a href="/api/admin/logout" />} variant="outline" size="icon" className="border-white/20 bg-transparent text-white hover:bg-white/10" aria-label="Keluar"><LogOut /></Button></div></div></header>
    <div className="mx-auto max-w-[1440px] px-5 py-7 sm:px-8">
      ...
      <Tabs defaultValue="orders">
        <TabsList className="mb-5"><TabsTrigger value="orders"><ShoppingBag /> Pesanan</TabsTrigger><TabsTrigger value="settings"><Settings2 /> Pengaturan</TabsTrigger></TabsList>
