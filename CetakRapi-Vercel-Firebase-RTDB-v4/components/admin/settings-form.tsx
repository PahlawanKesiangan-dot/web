...
export function SettingsForm({ initial }: { initial: PublicSettings }) {
  ...
  return <form onSubmit={submit} className="space-y-8">
    ...
    <section><h2 className="text-lg font-extrabold">Aturan upload</h2><p className="mt-1 text-sm text-muted-foreground">Jumlah lampiran per pesanan tidak dibatasi. Batas berikut berlaku untuk setiap file.</p><div className="mt-4 grid gap-5 sm:grid-cols-2"><Field><FieldLabel htmlFor="max-file">Ukuran maksimum per file</FieldLabel><div className="relative"><Input id="max-file" type="number" min={1} max={7} value={settings.maxFileMb} onChange={(e) => number('maxFileMb', e.target.value)} className="h-11 pr-12" /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">MB</span></div><FieldDescription>Maksimal 7 MB per file karena lampiran disimpan sebagai Base64.</FieldDescription></Field><Field><FieldLabel>Format yang diizinkan</FieldLabel><div className="flex flex-wrap gap-2">{types.map((type) => { const checked = settings.allowedTypes.includes(type); return <label key={type} className="flex min-h-10 cursor-pointer items-center gap-2 rounded-lg border bg-white px-3 py-2 text-sm font-semibold"><Checkbox checked={checked} onCheckedChange={(value) => setSettings({ ...settings, allowedTypes: value ? [...settings.allowedTypes, type] : settings.allowedTypes.filter((item) => item !== type) })} />{type.toUpperCase()}</label>; })}</div><FieldDescription>Pilih minimal satu format.</FieldDescription></Field></div></section>
    ...
    <div className="flex flex-col items-start gap-3 sm:flex-row sm:items-center"><Button type="submit" size="lg" className="h-11 w-full sm:w-auto" disabled={busy}>{busy ? <Loader2 className="animate-spin" /> : <Save />}Simpan pengaturan</Button>{saved && <span className="flex items-center gap-1.5 text-sm font-semibold text-emerald-700"><Check className="size-4" />Tersimpan</span>}</div>
  </form>;
}
